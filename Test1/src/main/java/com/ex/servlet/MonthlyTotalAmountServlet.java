...
public class MonthlyTotalAmountServlet extends HttpServlet {
 ...
 // SQL query to retrieve monthly total amounts for the selected year
    private static final String QUERY="SELECT MONTH(STR_TO_DATE(TransactionDate, '%d-%m-%Y')) AS Month, "
            + "SUM(TotalAmount) AS TotalAmount FROM tdtldb.bills WHERE YEAR(STR_TO_DATE(TransactionDate, '%d-%m-%Y')) = ? "
